...
public class ElevensBoard extends Board {
	...
	private List<Integer> findPairSum11(List<Integer> selectedCards) {
		...
		List<Integer> pairSum11Spots = new ArrayList<>();
		for (int sk1 = 0; sk1 < selectedCards.size(); sk1++) {
			int k1 = selectedCards.get(sk1).intValue();
			for (int sk2 = sk1 + 1; sk2 < selectedCards.size(); sk2++) {
				int k2 = selectedCards.get(sk2).intValue();
				if (cardAt(k1).pointValue() + cardAt(k2).pointValue() == 11) {
					pairSum11Spots.add(k1);
					pairSum11Spots.add(k2);
				}
			}
		}
		return pairSum11Spots;
	}
	...
	private List<Integer> findJQK(List<Integer> selectedCards) {
		...
		List<Integer> spotsJQK = new ArrayList<>();
		for (Integer kObj : selectedCards) {
			int k = kObj.intValue();
			if (cardAt(k).rank().equals("jack")) {
				spotsJQK.add(kObj);
			} else if (cardAt(k).rank().equals("queen")) {
				spotsJQK.add(queenSpot);
			} else if (cardAt(k).rank().equals("king")) {
				spotsJQK.add(kingSpot);
			}
		}
		return spotsJQK;
	}
	...
	private boolean playPairSum11IfPossible() {
		 List<Integer> pair = findPairSum11();
		 if(!(pair.isEmpty()){
	...
	private boolean playJQKIfPossible() {
		...
		if(foundJack && foundQueen && foundKing){
			deck.replaceSelectedCards(spots);
			return true;
		}
		return false; // REPLACE !
	}
